...
public static function get_primary_fields_id($form_id, $type)
    ...
    public static function get_results_for_last($interval, $form_id, $field_id, $field_value, $offset = 0, $limit = 999999, $sort_by = 'submission_id', $descending = false, $dates = null)
    {
        ...
        switch (strtolower($interval))
        {
            ...
            case 'custom':
                if(is_array($dates))
                {
                   if($dates['from'] != '' || $dates['upto'] != '')
                   {
                       ...
                       $interval_string = 'BETWEEN \''.$dates['from'].' 00:00:00\' AND \''.$dates['upto'].' 23:59:59\'';
            ...
            case 'read':
            {
                ...
                $read_status="AND `is_read`='1'";
                break;
            }
            case 'unread':
            {
                  
                $read_status="AND (`is_read`='0' OR `is_read`='')";
               break;
            }
            ...
            if(is_array($filter_tags)){ 
                foreach($filter_tags as $filter_tag):
                    switch(strtolower($filter_tag)){
                        ...
                        case 'pending offline payments': 
                            $submission_ids= self::get_submissions_by_payment_type($form_id,"'offline'");
                            $pending_submission_ids= self::get_submissions_payment_status($form_id,"'pending','Pending'");
    ...
    public static function get_submissions_with_note($form_id){
        ...
        $qry = "SELECT s.submission_id from $table_submissions s INNER JOIN  $table_notes n on s.submission_id=n.submission_id and s.form_id=$form_id";
    ...
    public static function get_submissions_payment_status($form_id,$status){
        ...
        $qry = "SELECT submission_id from $table_payment_logs where form_id=$form_id and status in ($status)"; 
        $submission_ids=$wpdb->get_col($qry);
    ...
    public static function get_submissions_by_payment_type($form_id,$type){
        ...
        $qry = "SELECT submission_id from $table_payment_logs where form_id=$form_id and pay_proc in ($type)"; 
        $submission_ids=$wpdb->get_col($qry);
     ...
     */
     public static function get_submission_read_count($form_id,$type,$count= true){
        ...
        if($count):
            $qry = "SELECT COUNT(*) as count FROM $table_name WHERE is_read=$type and form_id=$form_id";
            $count= $wpdb->get_var($qry); 
            return $count;
        else:
            $qry = "SELECT submission_id FROM $table_name WHERE is_read=$type and form_id=$form_id";
            $submissions_ids= $wpdb->get_col($qry);    
     ...
     public static function get_blocked_submission($form_id){
     	global $wpdb;
     	$table_name = RM_Table_Tech::get_table_name_for('SUBMISSIONS');
     	$query=  "SELECT submission_id FROM $table_name WHERE form_id=$form_id";
