    ...
    protected function add_payment_fields($form)
    {
        ...
        foreach ($payment_gateways as $payment_gw)
        {
            if ($payment_gw === 'stripe' && ($rm_env_requirements & RM_REQ_EXT_CURL))
            {
                ...
                /* <![CDATA[ */
                var rm_stripe_obj = {"stripe_pub_key":"<?php echo $stripe_pub_key; ?>"};
                /* ]]> */
                </script></pre>
                <?php
                echo RM_Utilities::enqueue_external_scripts('stripe_script', esc_url_raw('https://js.stripe.com/v1/'), array());
                echo RM_Utilities::enqueue_external_scripts('stripe_utility_script', plugin_dir_url(__DIR__) . '/js/stripe_payment_utility.js', array());
                
                $stripe_eles[] = new Element_Textbox(RM_UI_Strings::get('LABEL_STRIPE_CARD_NUMBER'), "", array("maxlength" => "20", "autocomplete" => "off", "class" => "card-number stripe-sensitive", "id" => "rm_stripe_card_number_".$f_id."_".$f_no,"labelStyle" => $this->form_options->style_label, "style" => $this->form_options->style_textfield, "value" => ""));
                $stripe_eles[] = new Element_Password(RM_UI_Strings::get('LABEL_STRIPE_CARD_CVC'), "", array("maxlength" => "4", "autocomplete" => "off", "class" => "card-cvc stripe-sensitive", "id" => "rm_stripe_card_cvv_".$f_id."_".$f_no,"labelStyle" => $this->form_options->style_label, "style" => $this->form_options->style_textfield, "value" => ""));
                $stripe_eles[] = new Element_Select(RM_UI_Strings::get('LABEL_STRIPE_CARD_MONTH'), "month", $months, array("class" => "card-expiry-month","labelStyle" => $this->form_options->style_label, "style" => $this->form_options->style_textfield, "id" => "rm_stripe_card_exp_month_".$f_id."_".$f_no));
                $stripe_eles[] = new Element_Select(RM_UI_Strings::get('LABEL_STRIPE_CARD_YEAR'), "year", $years, array("class" => "card-expiry-year","labelStyle" => $this->form_options->style_label, "style" => $this->form_options->style_textfield, "id" => "rm_stripe_card_exp_year_".$f_id."_".$f_no));
                $stripe_eles[] = new Element_HTML('<span class="payment-errors" id="rm_stripe_payment_errors_'.$f_id."_".$f_no.'"></span>');
                $stripe_eles[] = new Element_HTML('<div id="payment-tk_'.$f_id."_".$f_no.'"></div>');
        ...
        if (count($radio_array) > 1)
            $form->addElement(new Element_Radio(RM_UI_Strings::get('LABEL_SELECT_PAYMENT_METHOD'), "rm_payment_method", $radio_array, array('required' => 1, "value" => $active_payment_method, "class" => "rm_payment_method_select", "id" => "id_rm_payment_method_select", "style" => $this->form_options->style_textfield,"labelStyle" => $this->form_options->style_label, 'onclick' => 'setup_payment_method_visibility(jQuery(this).val(),'.$f_id.','.$f_no.')')));
        ...
        if($active_payment_method == 'stripe')
            $form->addElement( new Element_HTML('<div class="rm_stripe_fields" id="rm_stripe_fields_container_'.$f_id.'_'.$f_no.'">'));
        else
            $form->addElement( new Element_HTML('<div class="rm_stripe_fields" style="display:none" id="rm_stripe_fields_container_'.$f_id.'_'.$f_no.'">'));
        foreach ($stripe_eles as $ele)
            $form->addElement($ele);
