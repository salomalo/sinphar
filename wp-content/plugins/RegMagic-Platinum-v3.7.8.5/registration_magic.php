...
if (is_plugin_active_for_network($rmsilver)) {
    deactivate_plugins($rmsilver);
}
if (is_plugin_active($rmsilver)) {
    deactivate_plugins($rmsilver);
}
if (is_plugin_active_for_network($rmbasic)) {
    deactivate_plugins($rmbasic);
}
if (is_plugin_active($rmbasic)) {
    deactivate_plugins($rmbasic);
}
*/
if (!defined('RM_PLUGIN_VERSION')) {
    define('RM_PLUGIN_BASENAME', plugin_basename(__FILE__ ));
    define('RM_PLUGIN_VERSION', '3.7.8.5');
...
//Check for plugin requirements before proceeding
    ...
    function registration_magic_include_external_libs() {
        ...
        if (version_compare('5.4', $installed_php_version, '<') && ($rm_env_requirements & RM_REQ_EXT_CURL))
          require_once RM_EXTERNAL_DIR . 'c_contact/class_rm_constant_contact.php';
        ...
        if(is_array($pgws) && count($pgws) > 0) {
            ...
            if(RM_Utilities::is_ssl() && in_array('stripe',$pgws)) {
...
//Set up update check
    $rm_update_checker = new PluginUpdateChecker_2_1('http://registrationmagic.com/metadata.json', __FILE__, 'registrationmagic-gold');
