...
class WC_Points_Rewards_Manage_Points_List_Table extends WP_List_Table {
	...
	private function get_items() {
		...
		$query = "FROM {$wpdb->prefix}users as users LEFT JOIN {$wpdb->prefix}wc_points_rewards_user_points as points_table ON users.ID = points_table.user_id WHERE 1=1 AND points_table.points_balance != 0 {$where} GROUP BY points_table.user_id ORDER BY (points_table.points_balance+0) {$order}";
