...
$form->addElement(new Element_Checkbox("<b>" . RM_UI_Strings::get('LABEL_ALLOW_MULTILINE') . ":</b>", "field_is_multiline", array(1 => ""), array("disabled"=>"disabled","id" => "rm_field_multiline", "class" => "rm_field_multiline rm_input_type", "value" => $data->model->field_options->field_is_multiline, "longDesc"=>RM_UI_Strings::get('HELP_ADD_FIELD_ALLOW_MULTILINE'))));
$form->addElement(new Element_HTML('</div>'));    
 ...
 jQuery(document).ready(function () {
      ...
      jQuery("#rm_submit_btn").click(
            function (e) {
               ...
               //Date Range Validator Ends
               //Max length Validator starts
               ...
               //Custom Validation regex Validator starts
                ...
                if(validation=='custom'){
                    try {
                        ...
                        {
                        jQuery("#rm_validation_error_text").html("Wrong Regex! </br>Suggestion:-Remove Forward slash from start and end of the Regex.");
                        jQuery("#rm_validation_error_row").show();
                        e.preventDefault();
                        }
                        }
                     catch(en) {
                        jQuery("#rm_validation_error_text").html("Wrong Regex!.<br>Possible Error:-"+en.message );
                        jQuery("#rm_validation_error_row").show();
                        e.preventDefault();
                                }
                    }
