...
function cerber_admin_request(){
	...
	elseif (isset($_GET['lockdelete'])) {
		$ip = $_GET['lockdelete'];
		if (cerber_block_delete($ip)) cerber_admin_message(sprintf(__('Lockout for %s was removed','wp-cerber'),$ip));
	}
...
function cerber_show_activity($args = array(), $echo = true){
	...
	if ( $rows = $wpdb->get_results( $query ) ) {
		...
		foreach ($rows as $row) {
			...
			$activity = '<span class="actv' . $row->activity . '">'.$labels[ $row->activity ].'</span>';
			/*
			if ($row->activity == 50 ) {
...
function cerber_show_dashboard() {
	...
	$kpi_show = '<table id = "crb-kpi"><tr>'.$kpi_show.'</tr></table>';

	echo '<div class="cerber-margin">' . $kpi_show . '<p style="text-align: right; margin: 0;">in the last 24 hours</p></div>';
