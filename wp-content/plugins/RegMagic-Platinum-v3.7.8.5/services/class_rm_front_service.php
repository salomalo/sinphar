...
public function get_user_login_name() {
    ...
    public function get_users($request,$options){
        ...
        if(isset($request->req['action']) && $request->req['action']=='rm_load_front_users'):
            if(isset($request->req['timerange'])):
                $options['timerange']= $request->req['timerange'];
            endif;
            
            if(isset($request->req['form_id'])):
                $options['form_id']= $request->req['form_id'];
            endif;
            
            $options['page_number']= isset($request->req['page_number']) ? (int) $request->req['page_number'] : 1;
